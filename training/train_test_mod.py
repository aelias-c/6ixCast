...
def train(
        model, batch_size, dataloader, optimizer, prev_updates, device='cpu',
        writer=None):
    """
    ...
    for batch_idx, (data, target) in enumerate(tqdm.tqdm(dataloader)):
        ...
        if n_upd % 100 == 0:
            ...
            print(
                f'Step {n_upd:,} (N samples: {n_upd*batch_size:,}), Loss: '
                + f'{loss.item():.4f} (Recon: {output.loss_recon.item():.4f}, '
                + f'KL: {output.loss_kl.item():.4f}) Grad: {total_norm:.4f}')
...
def test(model, dataloader, cur_step, latent_dim, device='cpu', writer=None):
    """
    ...
    with torch.no_grad():
        for data, target in tqdm.tqdm(dataloader, desc='Testing'):
            data = data.to(device)
    ...
    print(
        f'====> Test set loss: {test_loss:.4f} (BCE: {test_recon_loss:.4f}, '
        + f'KLD: {test_kl_loss:.4f})')
    ...
    if writer is not None:
        ...
        # Log reconstructions
        writer.add_images(
            'Test/Reconstructions', output.x_recon.view(-1, 1, 28, 28),
